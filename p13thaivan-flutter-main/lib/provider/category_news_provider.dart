...
class CategoryNewsProvider {
  CategoryNewsRepository? regionRepo = GetIt.I.get<CategoryNewsRepository>();

  CategoryNewsProvider();

  ///
  /// Get all categoryNewss
  ///
  Future<void> all({
    required Function(List<CategoryNewsModel> categoryNewss) onSuccess,
  ...
  ///
  /// Insert categoryNews to database
  ///
  Future<void> add({
    required CategoryNewsModel data,
    required Function(CategoryNewsModel categoryNews) onSuccess,
    required Function(dynamic error) onError,
  }) async {
    ...
    if (apiResponse.response.statusCode! >= 200 &&
      ...
      onSuccess(CategoryNewsModel.fromJson(results as Map<String, dynamic>));
    } else {
      onError(apiResponse.error);
    }
  }
  ...
  ///
  /// Update categoryNews to database
  ///
  Future<void> update({
    required CategoryNewsModel data,
    required Function(CategoryNewsModel categoryNews) onSuccess,
  ...
  ///
  /// Delete categoryNews to database
  ///
  Future<void> delete({
    required String id,
    required CategoryNewsModel data,
  ...
  ///
  /// Get paginate categoryNewss "page": 1, "limit": 10
  ///
