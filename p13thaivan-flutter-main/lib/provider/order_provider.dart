...
class OrderProvider {
  OrderRepository? regionRepo = GetIt.I.get<OrderRepository>();
  ...
  Future<void> add({
    ...
    if (apiResponse.response.statusCode! >= 200 &&
      ...
      onSuccess(OrderModel.fromJson(results as Map<String, dynamic>));
    } else {
      onError(apiResponse.error);
    }
  }
  ...
  Future<void> updateData({
    ...
    final ApiResponse apiResponse = await regionRepo!.updateData(data);
    if (apiResponse.response.statusCode! >= 200 &&
        apiResponse.response.statusCode! <= 300) {
      // call back data success
      final results = apiResponse.response.data as dynamic;
